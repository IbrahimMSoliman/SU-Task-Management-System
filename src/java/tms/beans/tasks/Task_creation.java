...
@Named
@SessionScoped
public class Task_creation implements Serializable
    {
    @Inject
    ...
    public void open_new_window()
        ...
        conn.close();
        common_functions.getInstance().open_dialog("add_edit_task", 800, 700);
        }
    ...
    public void Search()
        ...
        try
            ...
            ResultSet rs = conn.query("select  \n"
                    ...
                    + " from task tt,users uu where tt.added_by=" + user_details.getUser_id() + " and uu.user_id=tt.added_by order by tt.adding_date desc");
            while(rs.next())
                {
    ...
    public class task_class
        ...
        public void edit()
            {
            process_type = "update";
            editable = t_status.equalsIgnoreCase("NEW");
            reset();
            connect conn = new connect();
