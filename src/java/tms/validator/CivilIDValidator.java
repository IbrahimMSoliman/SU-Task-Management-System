...
@FacesValidator("sis.civilIDValidator")
public class CivilIDValidator implements Validator, ClientValidator
    ...
    public void validate(FacesContext facesContext, UIComponent uIComponent, Object value) throws ValidatorException
        ...
        if(value == null)
            ...
            String sql = "select app.student_id from students app where ltrim(TRIM(BOTH ' ' FROM app.national_id),'0')='" + value.toString().replaceFirst("^0+(?!$)", "") + "'";//  app.NATIONAL_ID='"+value.toString().replaceAll(" ", "").trim()+"'";
            ...
            if(!other_student_id.trim().equals(""))
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Validation Error", "National ID (" + value.toString() + " belongs to another student (" + other_student_id + "))"));
            conn.close();
